...
export default function ProfessionPage({ content }: PageProps) {
  ...
  const metaTitle = `Jak napsat životopis ${content.name} - vzor ${currentYear} | ${SITE_NAME}`;
      ...
      <>
        {parts.filter(Boolean).map((part, index) => {
          if (part.startsWith("[blog]")) {
            ...
            return (
              <Link key={`blog-${index}`} className={styles.generatorLink} href="/cs/blog/jak-napsat-zivotopis">
                {label}
              </Link>
            );
          }
          if (part.startsWith("[preview]")) {
            ...
            return (
              <Link key={`preview-${index}`} className={styles.generatorLink} href="/cs/preview">
                {label}
              </Link>
            );
          }
