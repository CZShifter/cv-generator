...
const recommendedSkills = [
  ...
  "Znalosť CRM systémov (napr. SAP, Helios)",
  "Fakturácia",
  "Time management",
...
// --- NOVÉ: Definícia jazykových úrovní ako pole stringov ---
...
const CvForm_edit: React.FC<CvFormProps> = ({ data, onChange, selectedTemplate, isEditMode = false, id }) => {
  ...
  const handleSaveLanguage = () => {
    ...
    if (!languageForm.name.trim() || !languageForm.level) {
      alert("Prosím, zadajte názov jazyka a vyberte úroveň.");
      return;
    }
  ...
  // nový stav pro cache–buster
  const [photoVersion, setPhotoVersion] = useState(0);

  // při změně data.photo inkrementujeme verzi
  useEffect(() => {
    if (data.photo) {
  ...
  // složíme URL s parametrem verze
  const photoSrc = data.photo?.startsWith("data:")
    ? data.photo
    : `${data.photo}?v=${photoVersion}`;
  ...
  const handleSaveCroppedImage = (croppedImageBase64: string) => {
    onChange({ ...data, photo: croppedImageBase64, photoVersion });
    ...
    <div className={styles.formOuter}>
      ...
      {step === 0 && (
        <div className={styles.card}>
          <div className={styles.formTitle}>Kontaktné údaje</div>
          <div className={styles.rowGrid}>
            <div className={styles.inputGroup}>
              <label>Meno <HelpTooltip text="Ak máte titul pred menom, uveďte ho sem" /></label>
              <input className={styles.input} value={data.name} onChange={e => onChange({ ...data, name: e.target.value })} />
            </div>
            <div className={styles.inputGroup}>
              <label>Priezvisko <HelpTooltip text="Ak máte titul za priezviskom, uveďte ho sem" /></label>
              <input className={styles.input} value={data.surname} onChange={e => onChange({ ...data, surname: e.target.value })} />
            </div>
            <div className={styles.inputGroup}>
              <div className={styles.labelRow}>
                <label>Pracovné zameranie <HelpTooltip text="Uveďte, čím sa živíte, napr. Architekt" /></label>
            ...
            <div className={styles.inputGroup}>
              <div className={styles.labelRow}>
                <label>LinkedIn <HelpTooltip text="Napíšte svoje používateľské meno za in/" /></label>
                <label className={styles.switchWrap} style={{ cursor: "pointer" }}>
            ...
            <div className={styles.inputGroup}>
              <div className={styles.labelRow}>
                <label>Rok narodenia <HelpTooltip text="Rok narodenia je voliteľný" /></label>
                <label className={styles.switchWrap} style={{ cursor: "pointer" }}>
          ...
          <div className={styles.sectionSwitch}>
            <label className={styles.labelRow}>
              ...
              <button
                ...
                onClick={async () => {
                  trackGAEvent('click', 'generate', 'generovani_chatgpt_o_mne_sk');
                  setError(null);
                  if (!data.summary || data.summary.trim() === "") {
                    setError("Najprv napíšte názov pozície alebo krátky popis, čo má AI rozvinúť. Napríklad: Som programátor so záľubou v grafike.");
                    return;
                  }
                  setIsGenerating(true);
                  try {
                    const res = await fetch("/api/sk/generate-summary", {
                      method: "POST",
                    ...
                    if (json.summary) {
                      ...
                      setError("Generovanie sa nepodarilo.");
                    }
                  } catch {
            ...
            {error && (
              <div className={styles.errorPopup}>
                ...
                <button
                  ...
                  aria-label="Zavrieť chybové okno"
                >×</button>
              </div>
            )}
            <textarea rows={7}
              ...
              placeholder="Napíšte o sebe, alebo sa opíšte pár slovami a vygenerujte text pomocou AI..."
            />
          </div>
          <div className={styles.sectionSwitch}>
            <label className={styles.labelRow}>
              <span>Pridať fotku <HelpTooltip text="Fotka musí mať maximálne rozmery 1024 × 1024 px a veľkosť 2 MB" /></span>
              <span className={styles.switchWrap} style={{ cursor: "pointer" }}>
            ...
            <label className={styles.photoUpload}>
              ...
              {data.photo && data.showPhoto && (
                <ReloadableImage
                  ...
                  style={photoConfig?.shape === 'rect' ? { borderRadius: 0 } : {}}
                />
              )}
            </label>
          </div>
          <div className={styles.formButtons}>
            <button
              ...
              onClick={() => router.push(`/sk/zaplaceno/${id}`)}
            >
              Späť na stiahnutie životopisu
            </button>
      ...
      {step === 1 && (
        <div className={styles.card}>
          <div className={styles.formTitle}>Vzdelanie</div>
          <div className={styles.addBox3}>
            <div className={styles.addBoxCol}>
              <label className={styles.inlineLabel} htmlFor="degree-input">Dosiahnuté vzdelanie: <HelpTooltip text="Uvádzajte od najvyššieho" /></label>
              <input
                id="degree-input"
                className={styles.input}
                placeholder="Dosiahnuté vzdelanie"
                value={eduForm.level}
                onChange={e => setEduForm(f => ({ ...f, level: e.target.value }))}
              />
            </div>
            <div className={styles.addBoxCol}>
              <label className={styles.inlineLabel} htmlFor="field-input">Študijný odbor: <HelpTooltip text="Názov odboru" /></label>
              <input
                id="field-input"
                className={styles.input}
                placeholder="Názov odboru"
                value={eduForm.field}
                onChange={e => setEduForm(f => ({ ...f, field: e.target.value }))}
              />
            </div>
            <div className={styles.addBoxCol}>
              <label className={styles.inlineLabel} htmlFor="school-input">Názov školy: <HelpTooltip text="Názov školy" /></label>
              <input
                id="school-input"
                className={styles.input}
            ...
            <div className={styles.addBoxCol}>
              <label className={styles.inlineLabel} htmlFor="year-input">Rok: <HelpTooltip text="Rok ukončenia" /></label>
              <input
                id="year-input"
                className={styles.input}
      ...
      {step === 2 && (
        <div className={styles.card}>
          ...
          <div className={styles.addBox}>
            <div className={styles.addBoxCol}>
              <label className={styles.inlineLabel} htmlFor="cert-name">Názov kurzu: <HelpTooltip text="Názov kurzu" /></label>
              <input
                id="cert-name"
                className={`${styles.input} ${!data.showCertifications ? styles.disabled : ""}`}
                placeholder="Názov kurzu/certifikátu"
            ...
            <div className={styles.addBoxCol}>
              <label className={styles.inlineLabel} htmlFor="cert-place">Inštitúcia: <HelpTooltip text="Názov organizácie, ktorá kurz organizovala" /></label>
              <input
                id="cert-place"
                className={`${styles.input} ${!data.showCertifications ? styles.disabled : ""}`}
                placeholder="Organizátor/organizácia"
                disabled={!data.showCertifications}
            ...
            <div className={styles.addBoxCol}>
              <label className={styles.inlineLabel} htmlFor="cert-year">Rok ukončenia: <HelpTooltip text="Rok ukončenia kurzu" /></label>
              <input
                id="cert-year"
                className={`${styles.input} ${!data.showCertifications ? styles.disabled : ""}`}
                placeholder="Rok ukončenia"
            ...
            <div className={styles.addBoxActions}>
              ...
              <button type="button" className={styles.save} onClick={handleSaveCert} disabled={!data.showCertifications}>
                {certEditIdx === null ? "Pridať" : "Uložiť"}
              </button>
            </div>
          </div>
          ...
          <div className={styles.formTitle}>Jazyky</div>
            <div className={styles.CheckboxContainer}>
              <div className={styles.addBox4}>
                <input
                  className={styles.input}
                  placeholder="Zadajte jazyk (napr. Slovenský jazyk)"
                ...
                <div className={styles.addBoxActions}>
                  ...
                  <button type="button" className={styles.save} onClick={handleSaveLanguage}>
                    {languageEditIdx === null ? "Pridať" : "Uložiť"}
                  </button>
                </div>
              </div>
      ...
      {step === 4 && (
        <div className={styles.card}>
          <div className={styles.formTitle}>Zručnosti</div>
          <div className={styles.addBox2}>
            <input
              className={styles.input}
              placeholder="Zadajte zručnosť"
              value={skillForm}
            ...
            <div className={styles.addBoxActions}>
              ...
              <button type="button" className={styles.save} onClick={handleSaveSkill}>
                {skillEditIdx === null ? "Pridať" : "Uložiť"}
              </button>
            </div>
          </div>
      ...
      {step === 5 && (
        <div className={styles.card}>
          <div className={styles.formTitle}>Pracovné skúsenosti</div>
          <div className={styles.addBox} style={{ flexDirection: "column", alignItems: "stretch" }}>
            <div style={{ display: "flex", gap: "0.7rem", flexWrap: "wrap" }}>
              <input className={styles.input} placeholder="Pozícia" value={expForm.position} onChange={e => setExpForm(f => ({ ...f, position: e.target.value }))} />
              <input className={styles.input} placeholder="Spoločnosť" value={expForm.company} onChange={e => setExpForm(f => ({ ...f, company: e.target.value }))} />
            ...
            <div style={{ margin: "1rem 0 0.5rem 0" }}>
              ...
              <b>Body (popis pracovní pozice): <HelpTooltip text="Napíšte, čo bola Vaša pracovná náplň, alebo vygenerujte pomocou AI." /></b>
              <button
                type="button"
                ...
                onClick={async () => {
                trackGAEvent('click', 'generate', 'generovani_chatgpt_zkusenosti_sk');
                setExpError(null);
                if (!expForm.position || expForm.position.trim() === "") {
                  setExpError("Najprv vyplňte názov pracovnej pozície.");
                  return;
                }
                setIsExpGenerating(true);
                try {
                  const res = await fetch("/api/sk/generate-exp-points", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                  ...
                  if (json.points && Array.isArray(json.points)) {
                    ...
                    setExpError("Generovanie sa nepodarilo.");
                  }
                } catch {
                  setExpError("Chyba pri komunikácii so serverom.");
                }
                setIsExpGenerating(false);
              }}
              >
                {isExpGenerating ? <>Generujem{expDots}</> : "Generovať"}
              </button>
              {expError && (
                <div className={styles.errorPopup}>
                  {expError}
              ...
              {expForm.points.map((p, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "row", alignItems: "center", marginBottom: 5 }} >
                  <input
                    ...
                    placeholder="Napr. Spolupráca s projektovým manažérom na odhadoch časovej náročnosti úloh"
                    value={p}
            ...
            <div className={styles.addBoxActions}>
              ...
              <button type="button" className={styles.save} onClick={handleSaveExp}>
                {expEditIdx === null ? "Pridať skúsenosť" : "Uložiť"}
              </button>
            </div>
          </div>
          ...
          <div className={styles.formButtons}>
            <button type="button" className={styles.cancel} onClick={() => setStep(4)}>
              Späť
            </button>
            {/* Pokud jsme v editačním režimu, zobraz tlačítko Uložit změny, jinak Dokončit */}
            <button
              type="button"
              ...
              onClick={async () => {
                ...
                if (isEditMode) {
                  try {
                    ...
                    const res = await fetch("/api/sk/update-cv", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({
                        id,
                    ...
                    alert("Došlo k chybe pri spracovaní.");
                  } finally {
                    setIsProcessing(false);
                  }
                } else {
            ...
            >
              {isProcessing ? "Ukladám..." : isEditMode ? "Uložiť zmeny" : "Dokončiť"}
            </button>
          </div>
        </div>
      )}
      {expiredModalOpen && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <h2>Platnosť 24h vypršala!</h2>
            <p>Uplynula lehota 24 hodín na úpravu. Ak chcete životopis zmeniť, musíte ho zakúpiť znova.</p>
            <button className={styles.save} onClick={() => setExpiredModalOpen(false)}>Zavrieť</button>
          </div>
        </div>
      )}
    </div>
