...
export function getCvTemplate2Sections(data: CvData) {
  ...
  left.push(
    <div className={styles.nameSection} key="name">
      ...
      {data.showTitle && (
        <div className={styles["position-title"]}>{data.title}</div>
      )}
    </div>
  );
    ...
    data.experience.forEach((exp, i) => {
      left.push(
        <div className={styles.section} key={`exp-${i}`}>
          ...
          {i === 0 && <div className={styles["section-title"]}>PRACOVNÍ ZKUŠENOSTI</div>}
